...
Данный скрипт использует API сайта [devman](https://dvmn.org) для отправки уведомлений о результатах проверки работ ученику через Telegram бота. А также скрипт запускает откладочного бота, который будет сообщать о возникающих ошибках в работе первого.
...
- В папке репозитория создайте `.env` файл и укажите в нем следующие параметры:
    ...
    * `MAIN_BOT_TOKEN` - токен основного вашего Telegram бота
    * `DEBUG_BOT_TOKEN` - токен отладочного бота
